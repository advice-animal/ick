...
@click.pass_context
def find_projects(ctx, v, verbose, vmodule, target):
    """
...
@click.option("--target", default=".", help="Directory to modify")  # TODO path, existing
@click.pass_context
def list_hooks(ctx, v, verbose, vmodule, target):
    """
    Lists projects in the current repo, using current settings
    """
    verbose_init(v, verbose, vmodule)
...
@click.pass_context
def run(ctx, v, verbose, vmodule, dry_run: bool, yolo: bool, filters: list[str], target: str):
    """

...
class Runner:
    ...
    def iter_rule_impl(self, ignore_minimum_urgency: bool = False) -> Iterable[BaseRule]:
        name_filter = re.compile(self.rtc.filter_config.name_filter_re).fullmatch
        for rule in self.rules:
            if not ignore_minimum_urgency and rule.urgency < self.rtc.filter_config.min_urgency:
                continue
    ...
    def iter_tests(self) -> Iterable[tuple[BaseRule, tuple[str, ...]]]:
        ...
        for impl in self.iter_rule_impl(ignore_minimum_urgency=True):
